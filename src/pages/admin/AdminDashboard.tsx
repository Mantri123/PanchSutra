...
import { Users, Calendar, DollarSign, Activity, Bell, Settings, BarChart3, UserCheck, CheckCircle, Loader2 } from 'lucide-react';
...
const AdminDashboard: React.FC = () => {
  const { user } = useAuth();
  const { appointments, notifications, therapies } = useApp();
  ...
  const totalRevenue = appointments.filter(s => s.status === 'completed').reduce((sum, s) => sum + (therapies.find(t => t.id === s.therapyId)?.cost || 0), 0);
  ...
  const stats = [
    {
      ...
      bgColor: 'bg-blue-50',
      change: '+12% from last month' // This can be dynamic later
    },
    {
      ...
      bgColor: 'bg-green-50',
      change: `${appointments.filter(a => isToday(new Date(a.date)) && a.status === 'completed').length} completed`
    },
    {
      ...
      change: '+18% from last month' // This can be dynamic later
    },
    {
      ...
      change: '2 new this week' // This can be dynamic later
    }
  ];
  ...
  const quickActions = [
    ...
    },
    {
      ...
      description: 'Book a session for a patient',
      icon: Calendar,
      link: '/admin/schedule',
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-100 rounded-2xl p-6 border-2 border-blue-200 animate-in slide-in-from-right-5 duration-700 delay-700">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold text-green-600">{therapies.length}</span>
                </div>
                <div className="flex justify-between items-center">
